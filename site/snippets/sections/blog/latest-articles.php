...
<div class="container max-w-6xl px-4 py-12 mx-auto">
  <h1 class="py-8 mb-8 text-5xl text-center font-playfair"><?= t('blog.unsereBlogBeiträge') ?></h1>
  <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
    <?php foreach ($articles as $post): ?>
      <a href="<?= $post->url() ?>">
        <div class="overflow-hidden bg-white border ">
          ...
          <?php endif ?>
          <div class="p-4">
            <span class="px-3 py-1 capitalize bg-white border-2 rounded text-primary border-primary"><?= $post->category() ?></span>
            <h2 class="mt-4 mb-2 text-xl font-semibold line-clamp-2"><?= $post->title() ?></h2>
            <?php if ($post->description()->isNotEmpty()): ?>
              <p class="mb-4 text-gray-600 line-clamp-3"><?= $post->description() ?></p>
            <?php endif ?>
